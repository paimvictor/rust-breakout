...
async fn main() {
    let font_regular = load_ttf_font("res/Silkscreen-Regular.ttf").await.unwrap();
    let _font_bold = load_ttf_font("res/Silkscreen-Bold.ttf").await.unwrap();
    ...
    loop {
        ...
        match game_state {
            GameState::Menu => {
                draw_title_text("Press SPACE to start", font_regular);
            }
            GameState::Game => {
                let score_text = format!("Score: {}", score);
                let score_text_dim = measure_text(&score_text, Some(font_regular), 30u16, 1.0);
                draw_text_ex(
                    &score_text,
            ...
            GameState::LevelCompleted => {
                draw_title_text(&(format!("You win!! {} score", score)), font_regular);
                if is_key_pressed(KeyCode::Space) {
                    game_state = GameState::Menu;
                    reset_game(
                        &mut score,
            ...
            GameState::Dead => {
                draw_title_text(&(format!("You DIED!! :`( {} score", score)), font_regular);
            }
        }
